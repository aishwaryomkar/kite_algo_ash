...
low-priced names. NSE publishes the official constituent list as a CSV;
we cache it locally because the hosting path occasionally moves, and
...
def apply_liquidity_filter(symbols, fetcher):
    """Drop symbols below the turnover/price thresholds in config.py."""
    keep = []
    for sym in symbols:
        try:
            ...
            if avg_turnover >= config.MIN_AVG_TURNOVER and last_price >= config.MIN_PRICE:
                keep.append(sym)
        except Exception as e:
